        ...
        if base_features_path is not None:
            try:
                ...
                if not base_factors_file.exists():
                    ...
                    # Load features regardless of validation result
                    # (validation may fail for non-standard field names like 'high' vs '$high')
                    ...
                    if not validate_qlib_features(list(features.values())):
                        logger.warning(
                            f"Base feature validation failed for features loaded from {base_factors_file}. "
                            f"Features will still be used but may have issues."
                        )
