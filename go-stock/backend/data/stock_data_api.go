...
func (receiver StockDataApi) GetStockHistoryMoneyData(stockCode string) []models.StockMoneyDataHis {
	...
	// 使用缓存的 Cookie，通过 chromedp 获取 eastmoney 的 cookie 以绕过反爬
	cookieHeader, err := FetchEastMoneyCookiesViaChromedp("", time.Minute*3, quoteEastMoneyPage)
	if err != nil {
	...
	if err != nil {
		logger.SugaredLogger.Errorf("GetStockCodeRealTimeData error: %v", err)
	}
	body := string(resp.Body())
	//logger.SugaredLogger.Infof("resp:%s", body)
	vm := otto.New()
